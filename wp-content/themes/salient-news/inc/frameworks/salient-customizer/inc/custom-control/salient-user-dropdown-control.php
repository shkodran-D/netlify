            ...
            if( !empty ( $users ) )  {
                ...
                <label>
                    ...
                    <select <?php $this->link(); ?>>
                        ...
                        printf( // WPCS: XSS OK
                            '<option value="-1" %s>%s</option>',selected($default_selected, 1, false),esc_html__('Select','salient-news'));
                        foreach ( $users as $user ) {
                            printf ( // WPCS: XSS OK
                                '<option value="%s" %s>%s</option>', absint( $user->data->ID ), selected($this->value(), absint( $user->data->ID ), false), esc_html( $user->data->display_name, 'salient-news' ) ) ;
                        }
                        ?>
                    </select>
                </label>
