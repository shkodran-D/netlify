            ...
            if( !empty ( $salient_posts ) ) {
                ...
                <label>
                    ...
                    <select <?php $this->link(); ?>>
                        ...
                        printf('<option value="-1" %s>%s</option>', selected( $default_selected, 1, false), esc_html__('Select','salient-news') );
                        foreach ( $salient_posts as $salient_post ) {
                          printf( // WPCS: XSS OK
                            '<option value="%s" %s>%s</option>', absint( $salient_post->ID ), selected($this->value(), absint( $salient_post->ID ), false),  esc_html( $salient_post->post_title, 'salient-news' ) );
                        }
                        ?>
                    </select>
                </label>
